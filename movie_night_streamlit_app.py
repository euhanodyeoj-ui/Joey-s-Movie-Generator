...
def clean_cell_value(value: Any) -> Any | None:
    if value is None:
        return None
    try:
        if pd.isna(value):
...
def load_movies(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    cover_url_col = get_column_name(df, ["Cover URL", "Cover Image", "Poster URL", "Poster", "Image URL"])
    poster_path_col = get_column_name(df, ["Poster Path", "TMDB Poster Path"])
    tmdb_id_col = get_column_name(df, ["TMDB ID", "TMDb ID", "Movie ID"])
    ...
    for _, row in df.iterrows():
        ...
        poster_path = clean_text_value(row.get(poster_path_col)) if poster_path_col is not None else None
        cover_url = clean_text_value(row.get(cover_url_col)) if cover_url_col is not None else None
        if not cover_url and poster_path:
            cover_url = poster_url_from_path(poster_path, width=342)

        tmdb_id = None
        if tmdb_id_col is not None and clean_cell_value(row.get(tmdb_id_col)) is not None:
            try:
                tmdb_id = int(float(row.get(tmdb_id_col)))
            except Exception:
                tmdb_id = clean_text_value(row.get(tmdb_id_col))

        movies.append({
            "full_title": raw_title,
            "title": movie_title,
            "year": year,
            "format": clean_cell_value(row.get(format_col)) if format_col is not None else None,
...
def get_movie_poster_url(movie_title: str, year: int | None, api_key: str) -> str | None:
    """Look up poster art from TMDb when the spreadsheet did not already provide it."""
    if not api_key or not movie_title:
        return None

    url = "https://api.themoviedb.org/3/search/movie"

    # Try the most precise search first, then fall back to title-only.
    attempts: list[dict[str, Any]] = [{"api_key": api_key, "query": movie_title}]
    if year:
        attempts.insert(0, {"api_key": api_key, "query": movie_title, "year": year})
    ...
    for params in attempts:
        try:
            ...
            for result in data.get("results", []):
                url_from_result = poster_url_from_path(result.get("poster_path"))
                if url_from_result:
                    return url_from_result
        except Exception:
            continue
    return None


def poster_url_for_collection_movie(movie: dict[str, Any], api_key: str | None = None, allow_lookup: bool = False) -> str | None:
    """Prefer poster art already stored in the Excel file; only call TMDb as a fallback."""
    existing_cover = clean_text_value(movie.get("cover_url"))
    if existing_cover:
        return existing_cover

    existing_path = clean_text_value(movie.get("poster_path"))
    if existing_path:
        return poster_url_from_path(existing_path)

    if allow_lookup and api_key:
        return get_movie_poster_url(movie.get("title", ""), movie.get("year"), api_key)
...
def display_collection_tab(movies: list[dict[str, Any]], api_key: str) -> None:
    ...
    load_posters = st.checkbox(
        "🖼️ Show poster art on this shelf page",
        value=True,
        help="The app first uses poster URLs already saved in your Excel file. If a tape is missing artwork, it can ask TMDb for that one poster as a fallback.",
    )
    allow_tmdb_poster_fallback = st.checkbox(
        "🧟 Search TMDb for any missing posters",
        value=False,
        help="Optional fallback. Leave this off for the fastest shelves; turn it on only if a few movies are missing artwork from the spreadsheet.",
    )
    ...
    for row_start in range(0, len(visible_movies), cols_per_row):
        ...
        for col, movie in zip(cols, visible_movies[row_start:row_start + cols_per_row]):
            with col:
                ...
                poster_url = poster_url_for_collection_movie(
                    movie,
                    api_key=api_key,
                    allow_lookup=bool(load_posters and allow_tmdb_poster_fallback and api_key),
                ) if load_posters else None
                if poster_url:
...
def main() -> None:
    ...
    if active_section == "rental_counter":
        ...
        if st.button("📼 Ring the bell for 3 cursed staff picks", type="primary", width="stretch"):
