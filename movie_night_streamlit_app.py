...
<style>
    ...
    .stApp {
        background:
            radial-gradient(circle at 12% 5%, rgba(181,18,47,0.22), transparent 26%),
            radial-gradient(circle at 88% 10%, rgba(99,212,255,0.09), transparent 24%),
            radial-gradient(circle at 45% 95%, rgba(124,255,138,0.07), transparent 31%),
            repeating-linear-gradient(0deg, rgba(255,255,255,0.018), rgba(255,255,255,0.018) 1px, transparent 1px, transparent 6px),
            linear-gradient(180deg, #050308 0%, #110712 44%, #070407 100%);
        color: var(--paper);
    }
    ...
    header[data-testid="stHeader"] {
        background: linear-gradient(90deg, rgba(5,3,8,0.96), rgba(31,5,12,0.95));
        border-bottom: 1px solid rgba(255,49,95,0.45);
        backdrop-filter: blur(8px);
    }

    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, rgba(12,6,11,0.98), rgba(5,3,7,0.98));
        border-right: 1px solid rgba(255,49,95,0.25);
    }

    h1, h2, h3, .stMarkdown, p, label,
    [data-testid="stMetricValue"], [data-testid="stMetricLabel"] { color: var(--paper); }
    ...
    [data-testid="stTextInput"] input {
        ...
        border-color: rgba(255,49,95,0.25) !important;
    }

    .hero-card {
        border: 2px solid rgba(181,18,47,0.78);
        border-radius: 22px;
        padding: 1.25rem 1.4rem;
        margin-bottom: 1rem;
        background:
            radial-gradient(circle at 8% 20%, rgba(255,49,95,0.13), transparent 26%),
            linear-gradient(135deg, rgba(21,7,12,0.98), rgba(5,4,7,0.98)),
            repeating-linear-gradient(90deg, rgba(255,255,255,0.025), rgba(255,255,255,0.025) 1px, transparent 1px, transparent 18px);
        box-shadow: 0 0 26px rgba(181,18,47,0.22), inset 0 0 28px rgba(0,0,0,0.78);
    }
    ...
    .hero-title {
        ...
        text-shadow: 0 0 12px rgba(255,49,95,0.38), 0 0 28px rgba(124,255,138,0.08);
    }

    .hero-sub { font-size: 1rem; line-height: 1.5; margin-bottom: 0.7rem; color: #d8c998; }
    ...
    .shelf-rule {
        ...
        box-shadow: inset 0 2px 1px rgba(255,255,255,0.06), inset 0 -5px 11px rgba(0,0,0,0.62), 0 9px 18px rgba(0,0,0,0.38);
    }
    ...
    .vhs-card {
        ...
        border-radius: 18px;
        overflow: hidden;
        border: 1px solid rgba(255,49,95,0.20);
        background: linear-gradient(165deg, rgba(26,22,24,0.98), rgba(6,6,8,0.99));
        box-shadow: 0 12px 30px rgba(0,0,0,0.48), inset 0 1px 0 rgba(255,255,255,0.04);
        margin-bottom: 1rem;
    }

    .vhs-topband {
        height: 18px;
        background: linear-gradient(90deg, rgba(181,18,47,0.92), rgba(124,255,138,0.24), rgba(99,212,255,0.28));
        opacity: 0.96;
    }
    ...
    .vhs-poster-shell {
        ...
        overflow: hidden;
        box-shadow: inset 0 0 0 5px rgba(0,0,0,0.34), 0 6px 16px rgba(0,0,0,0.42);
    }

    .vhs-poster-shell img {
        width: 100%;
        height: auto;
        aspect-ratio: 2 / 3;
        object-fit: contain;
    ...
    .vhs-title {
        ...
        line-height: 1.18;
        margin: 0;
        text-shadow: 0 0 8px rgba(255,49,95,0.16);
    }

    .vhs-subtitle { font-size: 0.76rem; color: #bfb083; margin: 0; letter-spacing: 0.03em; }
    ...
    .vhs-match { color: var(--mold); font-size: 0.68rem; font-weight: 800; text-align: right; }
    ...
    .poster-missing {
        ...
        text-align: center;
        background: linear-gradient(180deg, #151015, #050505);
        color: #bfa96b;
        font-size: 0.72rem;
        padding: 0.6rem;
    }

    .browse-note { color: #c8b985; font-size: 0.9rem; margin-top: -0.2rem; margin-bottom: 0.7rem; }

    @media (max-width: 760px) {
        .vhs-inner { grid-template-columns: 1fr; }
        .vhs-poster-shell { max-width: 230px; margin: 0 auto; }
    }
</style>
"""
...
def render_vhs_card(movie: dict[str, Any], label: str, match_type: str | None = None) -> None:
    ...
    overview = crop_text(movie.get("overview"), 190)
    ...
    safe_release_date = html.escape(str(release_date))
    safe_match_type = html.escape(str(match_type)) if match_type else ""

    if poster_url:
        safe_poster_url = html.escape(str(poster_url), quote=True)
        poster_html = f'<img src="{safe_poster_url}" alt="{safe_title} poster" loading="lazy">'
    else:
        poster_html = '<div class="poster-missing">NO POSTER<br>FOUND</div>'

    pills = "".join([
        metric_pill(safe_year),
        metric_pill(html.escape(runtime_text) if runtime_text else None),
        metric_pill(html.escape(rating_text) if rating_text else None),
        metric_pill(html.escape(status_text) if status_text else None),
        metric_pill(html.escape(format_text) if format_text else None),
    ])

    match_html = f'<div class="vhs-match">{safe_match_type}</div>' if safe_match_type else ""
    ...
    <div class="vhs-card">
        ...
        <div class="vhs-inner">
            ...
            <div class="vhs-info">
                <div class="vhs-label">{safe_label}</div>
                <p class="vhs-title">{safe_title}</p>
                <p class="vhs-subtitle">{safe_release_date} • {safe_genres}</p>
                <div class="vhs-pill-row">{pills}</div>
                <div class="vhs-overview">{safe_overview}</div>
                <div class="vhs-foot">
