...
<style>
    ...
    div[data-testid="stVerticalBlockBorderWrapper"] {
        border-color: rgba(255,49,95,0.28) !important;
        background: linear-gradient(165deg, rgba(24, 13, 18, 0.72), rgba(6, 5, 8, 0.88)) !important;
...
def render_vhs_card(movie: dict[str, Any], label: str, match_type: str | None = None) -> None:
    """Render a movie as a compact VHS-style card using native Streamlit elements.

    This avoids raw nested HTML cards, which can leak closing tags like </div>
    in some Streamlit/browser combinations.
    """
    ...
    runtime_text = f"{movie.get('runtime')} min" if movie.get("runtime") else "Runtime unknown"
    rating_text = f"⭐ {movie.get('rating'):.1f}" if movie.get("rating") is not None else "No rating"
    status_text = movie.get("watch_status") or "Status unknown"
    format_text = str(movie.get("format")).strip() if movie.get("format") is not None and not pd.isna(movie.get("format")) else "Format unknown"
    poster_url = movie.get("poster_url")
    genres = crop_text(movie.get("genres"), 72)
    overview = crop_text(movie.get("overview"), 210)
    release_date = movie.get("release_date") or "Unknown release"

    with st.container(border=True):
        st.markdown(f"**{label}**")
        poster_col, info_col = st.columns([0.95, 1.45], vertical_alignment="top")
        ...
        with info_col:
            st.markdown(f"### {title}")
            st.caption(f"{release_date} • {genres}")
            st.markdown(
                f"""
                **{year}** · **{runtime_text}**  
                **{rating_text}** · **{status_text}**  
                **Format:** {format_text}
                """
            )
            if match_type:
